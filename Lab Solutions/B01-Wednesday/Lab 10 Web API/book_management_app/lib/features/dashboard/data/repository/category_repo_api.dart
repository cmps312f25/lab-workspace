...
class CategoryRepoApi implements CategoryRepository {
  ...
  Future<List<Category>> getCategories() async {
    ...
    if (response.statusCode != 200) {
      throw ("Unable to read the categories from the server");
    }

    // map
    final List<dynamic> categoriesMap = response.data;
    return categoriesMap.map((json) => Category.fromJson(json)).toList();
  }
  ...
  Future<Category?> getCategoryById(int id) async {
    ...
    if (response.statusCode != 200) {
      throw ("Unable to read the category with id $id from the server");
    }
